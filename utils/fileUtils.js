...
// import fsPromises from "fs/promises"; // Use fsPromises for promise-based APIs
...
//     throw new Error("Valid project ID is is required for ZIP extraction");
//   }

//   const extractedFilePaths = [];
//   const filePath = file.path;

//   console.log(`Checking ZIP file existence: ${filePath}`);
...
//       fs.createReadStream(filePath) // Use standard fs for createReadStream
...
//         await fsPromises.rm(zipExtractDir, { recursive: true });
...
//   const dir = path.join(UPLOADS_BASE_DIR, projectId, "Thumbnails");

//   console.log(`Checking thumbnail file existence: ${filePath}`);
//   if (!(await fsPromises.stat(filePath).catch(() => false))) {
...
//     const urlPath = `/Uploads/${projectId}/Thumbnails/${uniqueFileName}`;
...
//   const dir = path.join(UPLOADS_BASE_DIR, projectId, "Images");

//   console.log(`Checking image file existence: ${filePath}`);
//   if (!(await fsPromises.stat(filePath).catch(() => false))) {
//     throw new Error(`Temporary file not found: ${filePath}`);
//   }
...
//     console.log(`Renaming ${filePath} to ${newFilePath}`);
//     await fsPromises.rename(filePath, newFilePath);
//     const urlPath = `/Uploads/${projectId}/Images/${uniqueFileName}`;
...
//   const dir = path.join(UPLOADS_BASE_DIR, projectId, "Videos");

//   console.log(`Checking video file existence: ${filePath}`);
//   if (!(await fsPromises.stat(filePath).catch(() => false))) {
//     throw new Error(`Temporary file not found: ${filePath}`);
//   }
...
//     console.log(`Renaming ${filePath} to ${newFilePath}`);
//     await fsPromises.rename(filePath, newFilePath);
//     const urlPath = `/Uploads/${projectId}/Videos/${uniqueFileName}`;
...
//   console.log(`Checking source code file existence: ${file.path}`);
//   if (!(await fsPromises.stat(file.path).catch(() => false))) {
...
//   const dir = path.join(UPLOADS_BASE_DIR, projectId, "OtherFiles");

//   console.log(`Checking other file existence: ${filePath}`);
//   if (!(await fsPromises.stat(filePath).catch(() => false))) {
//     throw new Error(`Temporary file not found: ${filePath}`);
//   }
...
//     console.log(`Renaming ${filePath} to ${newFilePath}`);
//     await fsPromises.rename(filePath, newFilePath);
//     const urlPath = `/Uploads/${projectId}/OtherFiles/${uniqueFileName}`;
...
//     const finalPath =
//       typeof filePath === "string" ? path.join(UPLOADS_BASE_DIR, filePath.replace(/^\/Uploads/, "")) : null;

//     for (const p of [tempPath, finalPath].filter(Boolean)) {
//       try {
//         console.log(`Checking file for cleanup: ${p}`);
...
export const saveProfilePicture = async (file, userId) => {
  if (!userId || typeof userId !== "string") {
    throw new Error("Valid user ID is required for profile picture");
  }
  ...
  const dir = path.join(UPLOADS_BASE_DIR, "ProfilePictures", userId);

  console.log(`Checking profile picture file existence: ${filePath}`);
  if (!(await fsPromises.stat(filePath).catch(() => false))) {
    throw new Error(`Temporary file not found: ${filePath}`);
  }
  ...
  try {
    ...
    console.log(`Renaming ${filePath} to ${newFilePath}`);
    await fsPromises.rename(filePath, newFilePath);
    const urlPath = `/Uploads/ProfilePictures/${userId}/${uniqueFileName}`;
    console.log(`Saved profile picture ${file.originalname} to ${newFilePath} with URL: ${urlPath}`);
    return urlPath;
  } catch (error) {
    console.error(`Failed to save profile picture ${file.originalname}: ${error.message}`);
...
export const ensureUploadDirs = async () => {
  const uploadDirs = [
    ...
    path.join(UPLOADS_BASE_DIR, "SourceCode"),
    path.join(UPLOADS_BASE_DIR, "OtherFiles"),
    path.join(UPLOADS_BASE_DIR, "ProfilePictures"), 
  ];
